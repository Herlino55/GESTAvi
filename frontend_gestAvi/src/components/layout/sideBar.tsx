    ...
    <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
      <div className="h-16 flex items-center px-6 border-b border-slate-800 gap-3">
        ...
        <span className="font-bold text-xl text-white">
          Ferme<span className="text-blue-500">App</span>
        </span>
        <button 
      ...
      <div className="absolute bottom-0 w-full p-4 border-t border-slate-800 bg-slate-900">
        ...
        <button 
          onClick={logout} 
          className="w-full flex items-center justify-center gap-2 p-2 rounded border border-slate-700 hover:bg-slate-800 text-rose-400 text-sm transition-colors"
        >
