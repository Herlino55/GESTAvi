...
export async function GET(req: NextRequest) {
  ...
  const ventes = await prisma.vente.findMany({ where: { companyId: user.companyId }, include: { lot: true } });
...
export async function POST(req: NextRequest) {
  ...
  const vente = await prisma.vente.create({ data: { ...body, companyId: user.companyId } });
