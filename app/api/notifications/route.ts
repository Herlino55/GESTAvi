...
export async function GET(req: NextRequest) {
  ...
  const notifications = await prisma.notification.findMany({ where: { companyId: user.companyId } });
...
export async function POST(req: NextRequest) {
  ...
  const notification = await prisma.notification.create({ data: { ...body, companyId: user.companyId } });
