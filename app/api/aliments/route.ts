...
export async function GET(req: NextRequest) {
  ...
  const aliments = await prisma.aliment.findMany({ where: { companyId: user.companyId } });
...
export async function POST(req: NextRequest) {
  ...
  const aliment = await prisma.aliment.create({ data: { ...body, companyId: user.companyId } });
