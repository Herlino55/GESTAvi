...
export async function GET(req: NextRequest) {
  ...
  const transactions = await prisma.transaction.findMany({ where: { companyId: user.companyId }, include: { lot: true } });
...
export async function POST(req: NextRequest) {
  ...
  const transaction = await prisma.transaction.create({ data: { ...body, companyId: user.companyId } });
